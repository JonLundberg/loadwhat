...
- Emit every `LOAD_DLL_DEBUG_EVENT` as `RUNTIME_LOADED` (in order)
- Capture termination:
...
## 3) Windows default search order (single, fixed)
...
## 4) Output contract (line-oriented)
...
### Required tokens
- `RUN_START`, `RUNTIME_LOADED`, `RUN_END`
...
- `SUMMARY`
- `NOTE` (rare, for “not modeled” disclaimers)
...
SEARCH_PATH dll="foo.dll" order=5 path="C:\App\.\foo.dll" result="MISS"          # include only if CWD differs; otherwise omit
...
## 5) `imports` command (offline helper)
